...
export const paymentService = {
  ...
  getAllPayments: async (page = 0, size = 20, sort = 'createdAt,desc') => {
    const response = await api.get('/api/public/payments', {
  ...
  getPaymentById: async (paymentId) => {
    const response = await api.get(`/api/public/payments/${paymentId}`);
    return response.data;
  },

  // Get payments by order ID
  getPaymentsByOrderId: async (orderId) => {
    const response = await api.get(`/api/public/payments/order/${orderId}`);
    return response.data;
  },

  // Get payments by user ID
  getPaymentsByUserId: async (userId, page = 0, size = 20) => {
    const response = await api.get(`/api/public/payments/user/${userId}`, {
      params: { page, size },
    });
    return response.data;
  },
  ...
  getPaymentsByStatus: async (status, page = 0, size = 20) => {
    const response = await api.get(`/api/public/payments/status/${status}`, {
      params: { page, size },
    });
    return response.data;
  },
  ...
  getPaymentStatistics: async (startDate, endDate) => {
    const response = await api.get('/api/public/payments/statistics', {
