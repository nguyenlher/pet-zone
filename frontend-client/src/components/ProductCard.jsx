      ...
      <div className="relative block overflow-hidden">
        <Link to={`/products/${product.id}`} className="block">
          ...
          {product.status === 'OUT_OF_STOCK' && (
            <span className="absolute top-3 left-3 bg-red-500 text-white text-xs font-semibold px-2.5 py-1 rounded-full z-10">Out of Stock</span>
          )}
          {product.soldCount > 100 && (
            <span className="absolute top-3 left-3 bg-emerald-500 text-white text-xs font-semibold px-2.5 py-1 rounded-full z-10">Best Seller</span>
          )}
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 pointer-events-none z-10">
          <button 
            onClick={(e) => { e.preventDefault(); e.stopPropagation(); alert('Coming soon!'); }}
            className="w-9 h-9 rounded-full bg-white/90 flex items-center justify-center text-stone-600 hover:text-red-500 hover:bg-white shadow-md transition-all pointer-events-auto"
          >
            <Heart size={16} />
          </button>
        </div>
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-stone-100">
          ...
          <button
            ...
            onClick={(e) => { e.preventDefault(); addItem({ ...product, itemType: 'PRODUCT' }); }}
            title="Add to cart"
