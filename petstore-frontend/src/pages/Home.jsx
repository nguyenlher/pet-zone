...
import { pets, categories, testimonials } from '../data/mockData';
...
export default function Home() {
  const featuredPets = pets.filter(p => p.featured).slice(0, 8);
        ...
        <div className="container">
          ...
          <div className="category-grid">
            {categories.map((cat, i) => (
              <motion.div key={cat.id} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i} variants={fadeInUp}>
                <Link to={`/pets?category=${cat.slug}`} className="category-card">
                  <div className="category-image-wrap">
                    <img src={cat.image} alt={cat.name} loading="lazy" />
                  </div>
                  <div className="category-info">
                    <h3>{cat.name}</h3>
                    <span>{cat.count} pets</span>
                  </div>
                </Link>
              </motion.div>
            ))}
